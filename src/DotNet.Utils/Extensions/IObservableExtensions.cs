...
namespace DotNet.Utils
{
    public static class IObservableExtensions
    {
        public static IObservable<TResult> SelectAsync<TSource, TResult>(this IObservable<TSource> source, Func<TSource, Task<TResult>> selector)
        {
            ...
            {
                return selector(r).GetAwaiter().GetResult();
            });
        }
        public static IObservable<TSource> DoAsync<TSource>(this IObservable<TSource> source, Func<TSource, Task> selector)
        {
            return source.Do(r =>
            {
                selector(r).GetAwaiter().GetResult();
            });
        }
        public static IObservable<TSource> WithAsync<TSource>(this IObservable<TSource> source, Func<TSource, Task> selector, int maxConcurrent = 1)
        {
